...
class MPIrisArea:
    ...
    def process_frame_api(self, frame: np.ndarray) -> dict:
        """
        Process a single image frame and return tracking results as a dictionary.
        This method is purely for data extraction without any visualization.
        """
        h, w = frame.shape[:2]
        ...
        if results.multi_face_landmarks:
            ...
            result_data["cell_x"] = self.cell_x
            result_data["cell_y"] = self.cell_y
            result_data["cell_name"] = self.cell_name
            result_data["calibrated_angle"] = self.calibrated_angle
            result_data["gaze_point"] = self.gaze_point.tolist() if self.gaze_point is not None else None
            result_data["avg_eye_center"] = self.avg_eye_center.tolist() if self.avg_eye_center is not None else None
    ...
    def tracking_streaming(self, webcam_port: int, visible: bool = True):
        ...
        record_cap = cv2.VideoWriter(
            "eye_tracking_video.mp4", cv2.VideoWriter.fourcc(*"MJPG"), 30, (1280, 720)
        )
        while cap.isOpened():
            ret, frame = cap.read()
            if not ret:
            ...
            if visible:
                cv2.imshow("Calibrated Gaze Visualization", drawing_frame)
                record_cap.write(drawing_frame)
